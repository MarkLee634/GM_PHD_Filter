...
while (k < endTime)%k = timestep
    ...
    for j = 1:size(mk_minus_1,2) %should be equal to NUMBER_OF_DRONES
    ...
    numTargets_Jk_k_minus_1 = size(mk_minus_1,2); %should be equal to NUMBER_OF_DRONES since no death or spawn 
    ...
    for j = 1:numTargets_Jk_k_minus_1 %should be equal to NUMBER_OF_DRONES
        m_j = mk_k_minus_1(:,j);
        eta_j = H2 * m_j;
        eta = [eta, eta_j];
...
%         disp(s);
    ...
    maxValues = maxk(w_k, NUMBER_OF_DRONES)
    for i = 1:size(maxValues,2)
        I = [I, find(w_k == maxValues(i)) ];
    end
    I = sort(I);
    wk_minus_1 = w_k(I);
    mk_minus_1 = m_k(:,I);
    ...
    % states = w * m (TO DO)
    m_bar_k = [m_bar_k, mk_minus_1];
    w_bar_k = [w_bar_k, wk_minus_1];
    P_bar_k = [P_bar_k, Pk_minus_1];
